...
function resolveRound(oppAction: Action, myAction: Action): Outcome {
  switch (oppAction) {
    case Action.Rock:
      switch (myAction) {
        ...
        case Action.Scissors:
          return Outcome.Loss
        default:
          throw Error(`Invalid Action enumeration value: ${myAction as number}`)
      }
    case Action.Paper:
      switch (myAction) {
        case Action.Rock:
    ...
    default:
      throw Error(`Invalid Action enumeration value: ${oppAction as number}`)
  }
}
...
function findAction(action: Action, outcome: Outcome): Action {
  switch (action) {
    case Action.Rock:
      switch (outcome) {
        ...
        case Outcome.Win:
          return Action.Paper
        default:
          throw Error(`Unrecognized Outcome enumeration value: ${outcome as number}`)
      }
    case Action.Paper:
      switch (outcome) {
        case Outcome.Loss:
    ...
    default:
      throw Error(`Unrecognized Action enumeration value: ${action as number}`)
  }
}
